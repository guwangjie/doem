      ...
      <el-col :xs="24" :md="12">
        ...
        >
          ...
          <el-form-item label="是否显示" prop="hidden">
            <el-radio v-model="ruleForm.hidden" :label="parseInt(1)"
              >显示</el-radio
            >
            <el-radio v-model="ruleForm.hidden" :label="parseInt(2)"
              >隐藏</el-radio
            >
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  async mounted() {
    try {
      ...
      await this.getPermissionMenuDetail()
    } catch (e) {}
  },
  methods: {
    getOptions() {
      return permissionMenuOptions({ id: this.ruleForm.id })
        .then(res => {
    ...
    getPermissionMenuDetail() {
      return permissionMenuDetail({ id: this.ruleForm.id })
        .then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          permissionMenuEdit(this.ruleForm)
            .then(res => {
              this.$message({
                message: '保存成功',
                type: 'success',
                onClose: function() {
                  _this.$router.push(
                    '/permission/permission_menu/permission_menu_lst'
                  )
                }
              })
            })
